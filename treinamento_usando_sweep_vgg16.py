...
def denormalizar_imagem(img, arquitetura="vgg16"):
    img = img.copy()

    if arquitetura.lower() in ["vgg16"]:
...
def analise_distribuicao_probabilidades(y_true, y_probs, class_names, bins=50, figsize=(12, 5)):
    """
    ...
    # Encontra threshold pelo critério correto de Youden's J
    fpr, tpr, roc_thresholds = roc_curve(y_true, y_probs)
    youden_idx = np.argmax(tpr - fpr)
    best_thresh_youden = roc_thresholds[youden_idx]
...
def avaliar_threshold_com_curva_risco(y_true, y_probs, class_names, custo_fp=1.0, custo_fn=1.0):
    """
...
def visualizar_predicoes(model, dataset, class_names, num_images=16, threshold=0.5):
    ...
    for images, labels in dataset.take(1):
        ...
        for i in range(min(num_images, len(images))):
            ...
            # desfaz preprocess_input do VGG16
            img[..., 0] += 103.939
            img[..., 1] += 116.779
            img[..., 2] += 123.68
...
def analise_de_predicoes_incertas(model, dataset, class_names, threshold_range=(0.3, 0.7)):
    ...
    if len(uncertain_images) > 0:
        ...
        return fig
    return None
def coletar_preds_e_labels(model, dataset):
    y_true = []
    y_probs = []
...
def train():
    ...
    checkpoint_path = "melhor_modelo_vgg16.keras"
    ...
    val_ds = image_dataset_from_directory(
        ...
        batch_size=BATCH_SIZE, image_size=IMG_SIZE, shuffle=False, color_mode="rgb", seed=42
    ...
    # Congela todas menos as últimas N camadas
    ...
    callbacks = [
        ...
        WandbModelCheckpoint(filepath="modelos/modelo_vgg16.keras", monitor="val_loss", save_best_only=True)
    ]
    ...
    y_true_val, probs_val = coletar_preds_e_labels(melhor_modelo, val_ds)
    fpr_val, tpr_val, thresholds_val = roc_curve(y_true_val, probs_val)
    melhor_thresh = thresholds_val[np.argmax(tpr_val - fpr_val)]
    
    # teste
    y_true, probs = coletar_preds_e_labels(melhor_modelo, test_ds)
    ...
    y_true_cross, probs_cross = coletar_preds_e_labels(melhor_modelo, cross_val_ds)
    fpr_cross, tpr_cross, thresholds_cross = roc_curve(y_true_cross, probs_cross)
    melhor_thresh_cross = thresholds_cross[np.argmax(tpr_cross - fpr_cross)]
    preds_cross = (probs_cross > melhor_thresh_cross).astype(int)
    ...
    stats_nthudd, fig_dist_nthudd = analise_distribuicao_probabilidades(
        y_true_cross, probs_cross, CLASS_NAMES
    )
    ...
    threshold_custo, min_custo, fig_custo = avaliar_threshold_com_curva_risco(
        y_true, probs, CLASS_NAMES, custo_fp, custo_fn
    )
    ...
    if loss_val < 0.3 and overfit_loss < 0.1:
        melhor_modelo.save("melhor_modelo_vgg16.keras")
        art = wandb.Artifact(
            "melhor_modelo_vgg16",
            type="model",
            metadata={"val_loss": loss_val, "val_auc": auc_val, "overfit": overfit_loss}
        )
        art.add_file("melhor_modelo_vgg16.keras")
    ...
    print(f"Acurácia: {test_accuracy:.4f} | Precisão: {test_precision:.4f} | Recall: {test_recall:.4f} | F1: {test_f1:.4f}")
    print("========================================================")
    print("\n========================================================")
