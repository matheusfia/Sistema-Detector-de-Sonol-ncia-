...
from keras.layers import Dense, Dropout, GlobalAveragePooling2D,Conv2D, BatchNormalization, MaxPooling2D, Flatten
...
def denormalizar_imagem_custom(img):
    img = img.copy()
    if img.max() > 1.0:
        img = img / 255.0
    img = np.clip(img, 0, 1)
...
def logar_gradcam_final(modelo, dataset, class_names, num_imagens=4):
    """Gera e loga mapas Grad-CAM para o modelo binário (ResNet50 + sigmoid)."""
    try:
        # =========================================================
        ...
        if base_model is None:
            raise ValueError("ResNet50 não encontrado no modelo carregado")
        ...
        grad_model = tf.keras.models.Model(
            inputs=modelo.input,
            outputs=[ultima_camada.output, modelo.output]
        )
        ...
        for imagens, rotulos in dataset.take(1):
            for i in range(min(num_imagens, len(imagens))):
                ...
                img_visual = denormalizar_imagem_custom(img_visual)
...
def visualizar_predicoes(model, dataset, class_names, num_images=16, threshold=0.5):
    ...
    for images, labels in dataset.take(1):
        ...
        for i in range(min(num_images, len(images))):
            ...
            img = denormalizar_imagem_custom(img)
...
def train():
    wandb.init(project="cnn_customized")
    config = wandb.config
    ...
    checkpoint_path = "melhor_modelo_cnn.keras"
    ...
    for i in range(min(config.num_layers - 1, len(conv_configs))): 
        filters, l2_value = conv_configs[i]
        model.add(Conv2D(filters, (3,3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(l2_value)))
        model.add(BatchNormalization())
        model.add(MaxPooling2D(2,2))

    model.add(GlobalAveragePooling2D())
    model.add(Dense(512, activation='relu', kernel_regularizer=regularizers.l2(config.l2)))
    ...
    if loss_val < 0.3 and overfit_loss < 0.1:
        melhor_modelo.save("melhor_modelo_cn_customized.keras")
        art = wandb.Artifact(
            "melhor_modelo_cnn_customized",
            type="model",
            metadata={"val_loss": loss_val, "val_auc": auc_val, "overfit": overfit_loss}
        )
        art.add_file("melhor_modelo_cnn_customized.keras")
