...
from keras.layers import Dense, Dropout, GlobalAveragePooling2D,SpatialDropout2D
...
def train():
    wandb.init(project="mobilenet_v2")
    config = wandb.config
    ...
    checkpoint_path = "melhor_modelo_mobilenet_v2.keras"
    ...
    model = Sequential([
        base_model,
        SpatialDropout2D(config.dropout),
    ...
    melhor_modelo.save("melhor_modelo_mobilenet_v2.keras")

    # Criar e popular o JSON ANTES de criar o artifact
    with open("melhor_threshold_mobilenet_v2.json", "w") as f:
        json.dump({"threshold": float(melhor_thresh)}, f)

    # Criar artifact e adicionar arquivos
    art = wandb.Artifact(
        "melhor_modelo_mobilenet_v2",
        type="model",
        metadata={"val_loss": loss_val, "val_auc": auc_val, "overfit": overfit_loss}
    )
    art.add_file("melhor_modelo_mobilenet_v2.keras")
    art.add_file("melhor_threshold_mobilenet_v2.json") 
    
    # Logar artifact (finaliza tudo)
